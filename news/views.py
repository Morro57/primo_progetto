...
def queryBase(request):
    ...
    #nota per poter modificare la data di un articolo togliere la proprietà auto_now= True al field data model
    #poi dare i comandi makemigrations e migrate per applicare le mdoifiche al database
    articoli_mese_anno = Articolo.objects.filter(data__year=2023, data__month=1)
    ...
    .filter(articolo__visualizzazioni__gt=100): utilizza il metodo .filter() per filtrare i giornalisti
    in base al campon visualizzazioni nel modello Articolo. La notazione articoli_visualizzazioni indica
    ...
    In questo caso, ciò è utile perchè un giornalista potrebbe esserere associato a più articoli che soddisfano
    ...
    visualizzazioni= 50
    # Per mettere in AND le condizioni separarle con la vrirgola
    #18. scrivi quali articoli vengono selezionati
    articoli_condizioni_and = Articolo.objects.filter(giornalista__anno_di_nascita__gt=data, visualizzazioni__gte= visualizzazioni)
    
    # Per mettere in OR le condizioni usare Q
    ...
    context = {
        ...
        'articoli_condizioni_and': articoli_condizioni_and,
